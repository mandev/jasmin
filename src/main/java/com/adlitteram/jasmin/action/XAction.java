...
abstract public class XAction extends AbstractAction {
    ...
    public XAction(String actionName) {
        ...
        if (text == null) {
            ...
            if (text == null) text = actionName;
        }
       
        int index = text.indexOf('$');
        if (index != -1 && ((text.length() - index) > 1)) {
            text = text.substring(0, index).concat(text.substring(++index));
            putValue(MNEMONIC_KEY, Integer.valueOf((int) (text.charAt(index - 1))));
        }
        putValue("TEXT", text);

        // Label
        String labelName = XProp.get(actionName + ".label");
        if (labelName != null) putValue("LABEL", labelName);

        // Icon
        String iconName = XProp.get(actionName + ".ico");
        if (iconName != null) putValue(SMALL_ICON, GuiUtils.loadIcon(iconName));

        // Shortcut
        String keyName = XProp.get(actionName + ".key");
        if (keyName != null) putValue(ACCELERATOR_KEY, KeyStroke.getKeyStroke(keyName));

        // Tooltip
        String tipName = XProp.get(actionName + ".tip");
        if (tipName != null) putValue(SHORT_DESCRIPTION, tipName);
    }
