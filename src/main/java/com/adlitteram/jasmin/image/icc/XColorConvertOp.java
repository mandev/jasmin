 ...
 */
public class XColorConvertOp implements BufferedImageOp, RasterOp {
     ...
     * @param renderMode
     */
    public XColorConvertOp(int renderMode) {
        thisRenderState = renderMode;
        profileList = new ICC_Profile[0];
        /* 0 length list */
        this.hints = null;
    }

    public XColorConvertOp(RenderingHints hints, int renderMode) {
        thisRenderState = renderMode;
        profileList = new ICC_Profile[0];
        /* 0 length list */
        this.hints = hints;
     ...
     */
    public XColorConvertOp(ColorSpace cspace, RenderingHints hints, int renderMode) {
        if (cspace == null) {
     ...
     */
    public XColorConvertOp(ColorSpace srcCspace, ColorSpace dstCspace,
            RenderingHints hints, int renderMode) {
     ...
     */
    public XColorConvertOp(ICC_Profile[] profiles, RenderingHints hints, int renderMode) {
        if (profiles == null) {
    ...
    private BufferedImage ICCBIFilter(BufferedImage src,
        ...
        ICC_Profile srcProfile = ((ICC_ColorSpace) srcColorSpace).getProfile();
        ...
        if (srcProfile == destProfile) {
            ...
            if (noTrans) {
                Raster wr = src.getRaster();
                if (dest == null) {
                    dest = new BufferedImage(src.getColorModel(), wr.createCompatibleWritableRaster(), src.isAlphaPremultiplied(), null);
                }
                dest.getRaster().setRect(wr);
                return dest;
            }
        }
    ...
    private void updateBITransform(ICC_Profile srcProfile,
        ...
        int renderState = thisRenderState;
     ...
     * @param dest the destination <code>WritableRaster</code>, or
     * <code>null</code>
     * @return <code>dest</code> color converted from <code>src</code> or a new,
     * converted <code>WritableRaster</code> if <code>dest</code> * is
     * <code>null</code>
     * @exception IllegalArgumentException if the number of source or
     * destination bands is incorrect, the source or destination color spaces
     * are undefined, or this op was constructed with one of the constructors
     * that applies only to operations on BufferedImages.
     */
    @Override
     ...
     * @param destCM ColorModel of the destination. If null, an appropriate
     ...
     * <code>null</code> and this <code>ColorConvertOp</code> was created
     * without any <code>ICC_Profile</code> or <code>ColorSpace</code> defined
     * for the destination
     */
    @Override
     ...
     * bands from the specified <code>src</code>
     * @throws IllegalArgumentException if this <code>ColorConvertOp</code> was
     * created without sufficient information to define the <code>dst</code> *
     * and <code>src</code> color spaces
     */
    @Override
    ...
    private WritableRaster nonICCRasterFilter(Raster src,
        ...
        boolean srcIsFloat = (srcTransferType == DataBuffer.TYPE_FLOAT)
                || (srcTransferType == DataBuffer.TYPE_DOUBLE);

        boolean dstIsFloat = (dstTransferType == DataBuffer.TYPE_FLOAT)
                || (dstTransferType == DataBuffer.TYPE_DOUBLE);
